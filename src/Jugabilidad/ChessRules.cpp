...
Juego::Juego()
{
	...
	tab[5][1] = pieza_t(PEON, NEGRA);

	// Test enroque
	auto m = get_mov_permitidos(&(tab[5][1]), tab);
	cout << mov_print(m);
	cout << this->print();
...
bool Juego::haz_movimiento(int row_o, int col_o, int row_f, int col_f)
{
	...
	// comer al paso:
	if (mat.TAB[row_f][col_f] == COMER_AL_PASO) {
...
void Juego::aux_detectar_comer_al_paso(tablero_info_t& matriz, pieza_t peon, int row, int col)
{
	...
	if (numero_mov >=4) {
		...
		tablero_t t_o = historial[numero_mov];
		tablero_t t_1 = historial[numero_mov - 2];
		...
		for (int j = 0; j < COL_SIZE; j++) {
			if (t_o[row_ini][j].getForma() == PEON && t_o[row_ini][j].getColor() == !peon.getColor()) {
